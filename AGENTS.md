...
## MLC Doc Hub — localembed

Structured documentation lives in `.mlcai/`, maintained through the
`mlc-dochub` MCP server.

- **Project ID:** `localembed` — LocalEmbed — Local Embedding Server
- **Source directory:** `/mnt/data2tb/localembed` (read source files with native Read —
...
- **Use the project id `localembed`.** Never invent `localembed-mac`, and
